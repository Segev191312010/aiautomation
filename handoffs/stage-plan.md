...
Every user action shows feedback (toast), errors are caught gracefully, settings page works, users table exists with demo user, all tables have user_id, error responses use consistent `{error, detail}` format, request logging middleware active, pytest passes (if backend is accessible during session).
