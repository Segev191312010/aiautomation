...
- **RESOLVED on the integration branch:** `claude_worker` now uses the `mcp_server` contract (`tools_spec` list, one dictionary argument to `dispatch_tool_call`) and classifies the real `mark_declined` tool correctly. Keep the focused worker tests and the full backend gate green before enabling `CLAUDE_WORKER_ENABLED=true`.
