...
function TargetCard({ label, value, colorClass, bgClass, borderClass, large }: TargetCardProps) {
  return (
    <div className={`flex flex-col items-center justify-center p-4 rounded-xl border ${bgClass} ${borderClass}`}>
      <span className="text-[9px] font-sans font-medium text-zinc-500 uppercase tracking-wider mb-1.5">
        {label}
      </span>
      <span
        className={[
...
export default function PriceTargetsModule({ analyst, overview, loading = false }: Props) {
  if (!analyst && loading) {
    return (
      <section id="section-targets" className="card rounded-lg  p-6 animate-pulse">
  ...
  if (!hasTargets) {
    return (
      <section id="section-targets" className="card rounded-lg  p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xs font-sans font-medium text-zinc-400 tracking-wide">Price Targets</h3>
    ...
    <section id="section-targets" className="card rounded-lg  p-6">
      ...
      <div className="mb-1">
        ...
        <div className="flex justify-between text-[9px] font-mono text-zinc-500 mb-1">
          ...
          <span className="text-zinc-500 text-center flex-1 px-2 truncate">
            {currentPrice != null && (
        ...
        {/* The bar itself */}
        <div className="relative h-3 bg-gradient-to-r from-red-600/30 via-zinc-800 to-green-600/30 rounded-full">
          {/* Median marker — white dot */}
          {median != null && (
            <div
              className="absolute w-3 h-3 bg-zinc-900 border-2 border-[#FAF8F5] rounded-full top-0"
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3">
        {median != null && (
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-zinc-900 border border-[#FAF8F5] shrink-0" />
            <span className="text-[9px] font-sans text-zinc-500">Median</span>
          </div>
        )}
        {mean != null && (
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-indigo-600 border border-[#FAF8F5] shrink-0" />
            <span className="text-[9px] font-sans text-zinc-500">Consensus</span>
          </div>
        )}
        {currentPrice != null && (
          <div className="flex items-center gap-1.5">
            <div className="w-2 h-3 rounded-sm bg-amber-400 shrink-0" />
            <span className="text-[9px] font-sans text-zinc-500">Current Price</span>
          </div>
        )}
        {analyst.num_analyst_opinions != null && (
          <span className="text-[9px] font-sans text-zinc-500 ml-auto">
