...
function StatCard({ label, value, sublabel }: { label: string; value: string; sublabel?: string }) {
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3">
      <div className="text-[10px] font-sans uppercase tracking-wide text-gray-500">{label}</div>
      <div className="mt-1 text-2xl font-mono font-bold text-gray-900 tabular-nums">{value}</div>
      {sublabel && <div className="mt-1 text-[10px] font-sans text-gray-500">{sublabel}</div>}
    </div>
  )
}
...
function HolderRow({ holder }: { holder: Holder }) {
  ...
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3">
      <div className="min-w-0">
        <div className="truncate text-[12px] font-sans font-semibold text-gray-900" title={holder.name}>
          {holder.name}
        </div>
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-mono text-gray-500">
...
function OwnershipBar({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
      <div className="text-[10px] font-sans uppercase tracking-wide text-gray-500">Share Mix</div>
      <div className="mt-3 flex h-3 overflow-hidden rounded-full bg-white">
        ...
        {publicFloat > 0 && <div className="h-full bg-gray-200" style={{ width: `${publicFloat}%` }} />}
      </div>
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-[10px] font-sans text-gray-500">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="text-[10px] font-sans uppercase tracking-[0.18em] text-gray-500">{title}</div>
        {canToggle && (
          <button
            type="button"
            onClick={onToggle}
...
export default function OwnershipModule({ data, loading }: Props) {
  ...
  if (!data && loading) {
    return (
      <section className="card rounded-lg shadow-card p-6 animate-pulse">
        <div className="mb-5 h-3 w-28 rounded-lg bg-gray-100" />
  ...
  return (
    <section id="section-ownership" className="card rounded-lg shadow-card p-6">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-xs font-sans font-medium tracking-wide text-gray-500">Ownership</h3>
