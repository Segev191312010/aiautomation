...
function CompRow({ label, value, emphasize }: CompRowProps) {
  return (
    <div
      className={clsx(
        'flex justify-between items-center py-2 border-b border-zinc-800 last:border-0',
        emphasize && 'border-t border-zinc-800 pt-2.5',
      )}
    >
      <span className={clsx('text-[11px] font-sans', emphasize ? 'text-zinc-400 font-semibold' : 'text-zinc-400')}>
        {label}
      </span>
      <span className={clsx('text-[11px] font-mono tabular-nums', emphasize ? 'text-zinc-50 font-bold' : 'text-zinc-100')}>
        {value}
      </span>
    </div>
  )
...
function InfoCard({ label, value, sub }: InfoCardProps) {
  return (
    <div className="flex flex-col gap-1 rounded-lg bg-zinc-900 border border-zinc-800 p-3">
      <span className="text-[9px] font-sans text-zinc-400 uppercase tracking-wide">{label}</span>
      <span className="text-[13px] font-sans font-semibold text-zinc-50 leading-tight">{value}</span>
      {sub && <span className="text-[10px] font-sans text-zinc-400">{sub}</span>}
    </div>
  )
}
...
function EarningsCalendarCard({ earningsDetail }: EarningsCardProps) {
  ...
  return (
    <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 flex items-stretch gap-4">
      <div className="flex flex-col items-center justify-center rounded-lg bg-zinc-900 border border-amber-200 px-4 py-2 min-w-[64px]">
        <span className="text-[10px] font-sans font-semibold text-amber-700 uppercase tracking-wider">{month}</span>
        <span className="text-3xl font-mono font-bold text-zinc-50 tabular-nums leading-none">{day}</span>
        {dow && <span className="text-[9px] font-sans text-amber-700/70 mt-0.5">{dow}</span>}
      </div>

      <div className="flex flex-col justify-center gap-1 min-w-0">
        <span className="text-[9px] font-sans text-zinc-400 uppercase tracking-wide">Next Earnings</span>
        <span className="text-sm font-sans font-semibold text-zinc-50">{earningsDetail.next_date}</span>
...
export default function CompanyOverviewModule({ data, overview, earningsDetail, loading }: Props) {
  ...
  if (!data && loading) {
    return (
      <section id="section-company" className="card rounded-lg  p-6 animate-pulse">
        <div className="h-3 w-40 bg-zinc-800 rounded mb-5" />
        <div className="h-16 w-full bg-zinc-800 rounded-lg mb-4" />
        <div className="h-24 w-full bg-zinc-800 rounded-lg mb-4" />
  ...
  return (
    <section id="section-company" className="card rounded-lg  p-6 flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-sans font-medium text-zinc-400 tracking-wide">Company Overview</h3>
        <FreshnessTag fetchedAt={data.fetched_at} />
      </div>

      {overview?.description && <p className="text-[11px] font-sans text-zinc-400 leading-relaxed">{overview.description}</p>}

      {data.ceo && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3 rounded-lg bg-zinc-900 border border-zinc-800 p-4">
            <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-zinc-950 shrink-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-sans font-bold text-zinc-50 leading-snug">{data.ceo}</p>
              <p className="text-[10px] font-sans text-zinc-400">{data.ceo_title ?? 'Chief Executive Officer'}</p>
            </div>
          </div>

          {hasComp && (
            <div className="rounded-lg bg-zinc-900 border border-zinc-800 px-4 py-2">
              <span className="text-[9px] font-sans text-zinc-400 uppercase tracking-wide">Compensation</span>
              <div className="mt-2">
      ...
      {data.officers && data.officers.length > 0 && (
        <div>
          <button
            ...
            className="text-[10px] font-sans text-zinc-200 hover:text-zinc-50 transition-colors"
          >
          ...
          {showOfficers && (
            <div className="mt-2 flex flex-col">
              {data.officers.map((officer, idx) => (
                <div key={idx} className="flex justify-between items-baseline py-1.5 border-b border-zinc-800 last:border-0">
                  <span className="text-[10px] font-sans text-zinc-200 truncate max-w-[52%]">{officer.name}</span>
                  <span className="text-[9px] font-sans text-zinc-400 truncate max-w-[44%] text-right">{officer.title}</span>
                </div>
              ))}
            </div>
          )}
