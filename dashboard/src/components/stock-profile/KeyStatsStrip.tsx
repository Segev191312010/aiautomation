...
function Stat({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-zinc-900 border border-zinc-800 hover:bg-zinc-900/70 transition-colors">
...
function MarketCapCard({ value }: { value: number | null }) {
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-zinc-800 border border-zinc-800 hover:bg-zinc-900 transition-colors">
      <span className="text-[9px] font-sans text-zinc-500 uppercase tracking-wide">
        Market Cap
      </span>
      <span className="text-sm font-mono tabular-nums text-zinc-50 font-semibold">
        {fmtCompact(value)}
      </span>
    </div>
  )
...
function WeekRangeCard({
  ...
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-zinc-900 border border-zinc-800 hover:bg-zinc-900/70 transition-colors">
      ...
      <span className="text-xs font-mono tabular-nums text-zinc-100">
        {lowStr} — {highStr}
      </span>
      {/* Range bar */}
...
export default function KeyStatsStrip({ data, companyInfo, loading, currentPrice }: Props) {
  ...
  return (
    <section id="section-stats" className="card rounded-lg  p-6">
