  ...
  return (
    <section className="card rounded-2xl  p-3">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-sans font-medium text-zinc-400">Sector Projections</h3>
      ...
      {sorted.length === 0 ? (
        <div className="mt-3 text-[11px] font-mono text-zinc-500">No sector projections yet.</div>
      ) : (
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
          {sorted.map((row) => (
            <article key={row.sector} className="card rounded-2xl border border-zinc-800 p-2.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-sans text-zinc-400">{row.sector}</span>
                <span className={`text-[10px] font-mono ${directionClass(row.direction)}`}>{row.direction}</span>
              </div>
              <div className="mt-1 text-sm font-mono text-zinc-100">{row.score.toFixed(2)}</div>
            </article>
          ))}
        </div>
      )}
