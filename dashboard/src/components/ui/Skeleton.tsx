...
export default function Skeleton({ className = '', width, height }: SkeletonProps) {
  return (
    <div
      className={`animate-pulse bg-terminal-muted/30 rounded-2xl ${className}`}
      style={{ width, height }}
    />
  )
}
...
export function SkeletonCard({ className = '' }: { className?: string }) {
  return (
    <div className={`glass rounded-2xl shadow-glass p-4 ${className}`}>
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex gap-4">
          {Array.from({ length: cols }).map((_, j) => (
            <Skeleton key={j} className="h-4 flex-1 rounded-xl" />
          ))}
        </div>
      ))}
    </div>
