...
const INPUT_CLS =
  ...
  'font-sans text-sm text-zinc-100 placeholder:text-zinc-500 ' +
  'focus:outline-none focus:border-indigo-600/60 focus:ring-1 focus:ring-indigo-300 transition-all'

const SELECT_CLS =
  'px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-xl ' +
  'font-sans text-sm text-zinc-100 ' +
  'focus:outline-none focus:border-indigo-600/60 focus:ring-1 focus:ring-indigo-300 transition-all'

const LABEL_CLS = 'block text-[11px] font-sans font-semibold text-zinc-400 tracking-wider uppercase mb-1.5'

// ── Operator color map ────────────────────────────────────────────────────────

function operatorColor(op: string): string {
  if (op === '>' || op === '>=') return 'text-emerald-400'
  if (op === '<' || op === '<=') return 'text-red-400'
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="overflow-y-auto flex-1 px-5 py-5 space-y-5">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs font-sans text-zinc-500">
                {alertType === 'one_shot'
            ...
            {testResult !== null && (
              ...
              >
                {/* Status dot */}
                <span className={`mt-0.5 w-1.5 h-1.5 rounded-full shrink-0 ${testResult.triggered ? 'bg-emerald-600' : 'bg-zinc-600'}`} />
                <div>
            ...
            {error !== null && (
              <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl border bg-red-500/10 border-red-300 text-red-400 text-xs font-sans">
          ...
          {/* ── Sticky footer ──────────────────────────────────────────── */}
          <div className="px-5 py-4 border-t border-zinc-800 shrink-0 flex items-center justify-between gap-3 bg-zinc-900">
            {/* Test button */}
            <button
              type="button"
              onClick={handleTest}
              ...
              className={[
                'flex items-center gap-1.5 px-3 py-2 rounded-xl border border-zinc-800',
                'text-xs font-sans font-medium text-zinc-400',
                'hover:text-zinc-100 hover:border-zinc-800',
                'disabled:opacity-40 transition-colors',
              ].join(' ')}
            >
              {testing ? (
                <>
                  <span className="w-3 h-3 rounded-full border-2 border-zinc-700 border-t-transparent animate-spin" />
                  Testing...
                </>
              ) : (
                <>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={onClose}
                className={[
                  'px-4 py-2 rounded-xl border border-zinc-800',
                  'text-xs font-sans font-medium text-zinc-400',
                  'hover:text-zinc-100 hover:border-zinc-800',
                  'transition-colors',
                ].join(' ')}
